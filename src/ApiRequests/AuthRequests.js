...
export const login = async(dispatch,user) => {
    ...
    try {
        const res = await axios.post("http://localhost:5000/api/v1/auth/login-user", user,{withCredentials:true});
...
export const logout = async (dispatch, id) => {
  ...
  try {
    const res = await axios.post("http://localhost:5000/api/v1/auth/logout", id,{ withCredentials: true });
    console.log(res);
